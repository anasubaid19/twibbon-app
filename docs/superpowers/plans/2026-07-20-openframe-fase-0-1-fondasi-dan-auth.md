...
> **Indeks wajib.** `getAuthTables()` milik Better Auth menandai
> `session.userId`, `account.userId`, dan `verification.identifier` sebagai
> `index: true`, dan generator migrasi bawaannya membuat indeks itu. Karena
> schema di sini ditulis tangan, indeksnya harus ditulis tangan juga —
> kalau tidak, setiap pencarian sesi milik user akan sequential scan begitu
> tabelnya tumbuh. Menambahkannya sekarang gratis; menambahkannya ke tabel
> besar nanti mengunci tabel.
